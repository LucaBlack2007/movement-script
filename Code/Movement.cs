...
/*
    ...
    -------------------------------------------------
        ...
        !!! all binds can be changed on LINE 41 - 58 !!!
...
public class Movement : MonoBehaviour {
    ...
    public Texture2D coinTexture1;
    public Texture2D coinTexture2;
    public Texture2D badguyTexture1;
    public Texture2D badguyTexture2;
    ...
    float timer;
    float updateTimer;
    public static string timerPretty;
    ...
    void Start() {
        ...
        cube = Instantiate(prefabObj3, new Vector3(0,0,0), new Quaternion(0,0,180,0));
        ...
        buff = Instantiate(prefabObj2, new Vector3(0,0,0), Quaternion.identity);
        ...
        buffColor.material.mainTexture = coinTexture1;
        cubeColor.material.mainTexture = badguyTexture1;
    ...
    void Update() {  
        ...
        */
        ...
        if (Input.GetKey(Bind.RESTART)) {
            if (!inSession) {
                ...
                canMove = true;
                canBoost = false;
                mainColor.material.mainTexture = playerTexture1;
                cubeColor.material.mainTexture = badguyTexture1;
                for (int i = 0; i < 10; i++) print(" "); 
                this.name = $"Health: {health}"; 
        ...
        if (Input.GetKey(Bind.WIN) && isDEV) {
            death(false);
            this.name = "You Win!";
        }
        ...
        void spawnFruit() { 
            ...
            if (powerUp) 
                buffColor.material.mainTexture = coinTexture2;
            else 
                buffColor.material.mainTexture = coinTexture1;
        ...
        */
        ...
        if (inSession) this.name = health > 0 ? ($"Health: {health}") : "Game Over!";
        
        timer = inSession ? Mathf.Round(Time.time - updateTimer) : 0f;
        
        //int sec_num = (int)timer;
        float hours = Mathf.Floor(timer / 3600);
        float minutes = Mathf.Floor((timer - (hours * 3600)) / 60);
        float seconds = timer - (hours * 3600) - (minutes * 60);
        string h = "", m = "", s = "";
    ...
    void OnTriggerEnter(Collider other) {
        ...
        if (tag == "Buff") {
            ...
            if (counter % 3 == 0 && !canBoost) {
                mainColor.material.mainTexture = playerTexture2;
            ...
            if (powerUp) {
                powerUp = false;
                pickedUp = true;
                cubeColor.material.mainTexture = badguyTexture2;
            } else {
                pickedUp = false;
                cubeColor.material.mainTexture = badguyTexture1;
            }
